...
import io.lokopay.exception.InvalidRequestException;
...
public abstract class ApiResource extends LokoObject implements LokoActiveObject {
    public static final Charset CHARSET = StandardCharsets.UTF_8;
    private static ResponseGetter globalResponseGetter = new LokoResponseGetter();
    private transient ResponseGetter responseGetter;

    public static final Gson INTERNAL_GSON;

    static {
        INTERNAL_GSON = createGson(false);
    }

    public static final Gson GSON = createGson(true);

    public static void setLokoResponseGetter(ResponseGetter srg) {
        ApiResource.globalResponseGetter = srg;
    }

    protected static ResponseGetter getGlobalResponseGetter() {
        return ApiResource.globalResponseGetter;
    }

    @Override
    public void setResponseGetter(ResponseGetter srg) {
        responseGetter = srg;
    }

    protected ResponseGetter getResponseGetter() {
...
//                        .registerTypeAdapterFactory(new LokoCollectionItemTypeSettingFactory())
                        .addReflectionAccessFilter(
                                new ReflectionAccessFilter() {
                                    ...
                                    public ReflectionAccessFilter.FilterResult check(Class<?> rawClass) {
                                        if (rawClass.getTypeName().startsWith("io.lokopay.")) {
        ...
        if (shouldSetResponseGetter) {
            builder.registerTypeAdapterFactory(new LokoResponseGetterSettingTypeAdapterFactory());
        }
